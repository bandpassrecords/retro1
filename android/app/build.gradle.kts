...
// Try both locations: key.properties (relative to android/) and android/key.properties (from Flutter root)
val keystorePropertiesFile = rootProject.file("key.properties").takeIf { it.exists() }
    ?: rootProject.parent?.file("android/key.properties")?.takeIf { it.exists() }
    ?: null
        ...
        create("release") {
            if (keystorePropertiesFile != null && keystorePropertiesFile.exists()) {
                ...
                if (keystoreFile != null) {
                    // storeFile in key.properties can be:
                    // - "app/release.keystore" (relative to android/) -> android/app/release.keystore
                    // - "release.keystore" (relative to android/app/) -> android/app/release.keystore
        ...
        release {
            ...
            if (keystorePropertiesFile != null && keystorePropertiesFile.exists()) {
                ...
                if (releaseSigningConfig.storeFile != null && releaseSigningConfig.storeFile!!.exists()) {
                    ...
                    val storeFile = keystoreProperties["storeFile"] as String? ?: "not specified"
                    throw GradleException(
                        "Release signing config is not properly configured. " +
                        "Please check key.properties and ensure the keystore file exists. " +
                        "storeFile in key.properties: $storeFile"
                    )
                }
            } else {
                throw GradleException(
                    "Release builds require a keystore. " +
                    "Please create key.properties (in android/ directory) and android/app/release.keystore. " +
